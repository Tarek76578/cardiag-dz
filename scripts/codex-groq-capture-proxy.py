...
def token_estimate(v):
    raw=v if isinstance(v,(bytes,bytearray)) else json.dumps(v,ensure_ascii=False,separators=(",",":")).encode(); return max(1,(len(raw)+3)//4)
...
def is_tool_item(item):
    ...
    t=str(item.get("type","")).lower(); return "tool" in t or t in {"function_call","function_call_output","computer_call","computer_call_output"}
def compact_input(obj,max_tokens):
    ...
    while i<len(inp):
        x=inp[i]
        if item_role(x) in ("system","developer"): protected.append(x); i+=1; continue
        if is_tool_item(x):
            unit=[x]; j=i+1
    ...
    for unit in reversed(units):
        candidate=selected+unit; trial=dict(obj); trial["input"]=protected+[z for u in reversed(candidate) for z in u]
...
def duration(v):
    ...
    n=float(m.group(1)); u=m.group(2) or "s"; return n/1000 if u=="ms" else n*60 if u=="m" else n
    ...
    def do_POST(self):
        ...
        for mi,model in enumerate(models):
            ...
            while True:
                ...
                budget=max(512,ceiling-obj["max_output_tokens"])
                ...
                if estimated+obj["max_output_tokens"]>ceiling:
                    ...
                    if new_out<obj["max_output_tokens"]:
                        obj["max_output_tokens"]=new_out; body=json.dumps(obj,ensure_ascii=False,separators=(",",":")).encode(); estimated=token_estimate(body)
                if estimated+obj["max_output_tokens"]>ceiling:
                    obj,changed=compact_input(obj,max(512,ceiling-obj["max_output_tokens"]))
                    body=json.dumps(obj,ensure_ascii=False,separators=(",",":")).encode(); estimated=token_estimate(body); compacted=compacted or changed
                ...
                if status==413 and attempts<MAX_RETRIES:
                    ...
                    obj,changed=compact_input(obj,max(512,estimated//2))
