...
Independent implementation inspired by public architecture ideas in the MIT-
licensed Free Claude Code project: canonical failure classes, bounded retries,
...
class StreamCommittedError(RuntimeError):
    """The upstream stream failed after HTTP output was already committed."""
...
def model_options(provider: str, requested: str | None) -> list[str]:
    ...
    preferred = str(PROVIDERS[provider]["model"] or "")
...
def upstream_call(provider: str, body: dict[str, Any], model: str, accept: str | None):
    ...
    headers = {"Authorization": f"Bearer {PROVIDERS[provider]['key']}", "Content-Type": "application/json", "Accept": accept or "application/json"}
...
def forward_response(handler: BaseHTTPRequestHandler, upstream: Any) -> None:
    with upstream as response:
        ...
        try:
            ...
            raise StreamCommittedError(str(exc)) from exc
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/health":
            ...
            return
        if path == "/v1/models":
            data = [{"id": model_for(name, None), "object": "model", "owned_by": name} for name in ORDER if name in PROVIDERS and provider_ready(name, True)]
    ...
    def do_POST(self):
        ...
        for provider in candidate_providers(requested_provider, codex_request):
            if provider in {entry.split("/", 1)[0] for entry in tried}:
                continue
            with provider_semaphores[provider]:
                ...
                for model in options:
                    ...
                    try:
                        ...
                        return
                    except StreamCommittedError as exc:
                        mark_failure(provider, "stream_committed_failure")
                        print(f"STREAM_COMMITTED_FAILURE provider={provider} error={exc}", flush=True)
                        return
                        ...
                        if kind in ("rate_limit", "transient") and attempts < MAX_ATTEMPTS:
