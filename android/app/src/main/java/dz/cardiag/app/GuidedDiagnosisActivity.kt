...
private fun GuidedDiagnosisScreen(modelId: String?, modelName: String, initialCode: String) {
    ...
    fun load() {
        ...
        if (normalized.isBlank()) {
            errorMessage = "Entrez un code DTC, par exemple P0301."
            guide = null
            searched = false
            return
        }
        scope.launch {
            ...
            runCatching {
                SupabaseClient.client.from("diagnostic_codes")
                    .select(Columns.list("id", "code", "system", "category", "severity", "description_fr", "causes_fr", "diagnostic_steps_fr", "repair_summary_fr")) {
                ...
                guide = it
                if (it == null) errorMessage = "Code $normalized introuvable dans la base DTC."
            }.onFailure {
                errorMessage = "Impossible de charger le DTC depuis Supabase: ${it.message ?: "erreur réseau"}"
            }
            loading = false
        }
    }
    ...
    fun runAi() {
        ...
        if (normalized.isBlank() || guide == null) {
            errorMessage = "Chargez d'abord un DTC valide."
            return
        }
        scope.launch {
            aiLoading = true
            ...
            runCatching {
                val user = SupabaseClient.client.auth.currentSessionOrNull()?.user
                    ?: error("AUTH_REQUIRED")
                val session = SupabaseClient.client.from("diagnostic_sessions")
                    .insert(
                        DiagnosticSession(
                            userId = user.id,
                ...
                val response = SupabaseClient.client.functions.invoke(
                    function = "diagnose_ai",
                    body = buildJsonObject {
                        put("session_id", sessionId)
                        put("codes", buildJsonArray { add(JsonPrimitive(normalized)) })
                ...
                if (json["ok"]?.toString() != "true") {
                    error(json["error"]?.toString()?.trim('"') ?: "AI diagnostic failed")
                }
                raw
            }.onSuccess { aiResult = it }
                .onFailure {
                    errorMessage = when (it.message) {
                        "AUTH_REQUIRED" -> "Connectez-vous pour utiliser le diagnostic AI."
                        "SESSION_CREATE_FAILED" -> "Impossible de créer la session de diagnostic."
                        else -> it.message ?: "Erreur du moteur AI."
                    }
                }
            aiLoading = false
        }
    ...
    Scaffold(topBar = { TopAppBar(title = { Text("Diagnostic guidé") }) }) { padding ->
        LazyColumn(
            ...
            item {
                Card(Modifier.fillMaxWidth()) {
                    Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                        ...
                        Text(
                            "Entrez un DTC connu pour consulter la base CarDiag, puis lancez l'analyse AI. Aucun ELM327 n'est nécessaire pour cette étape.",
                            color = MaterialTheme.colorScheme.onSurfaceVariant
                        )
                    }
                }
            ...
            item {
                Button(
                    onClick = ::load,
                    enabled = code.isNotBlank() && !loading && !aiLoading,
                    modifier = Modifier.fillMaxWidth()
                ) { Text(if (loading) "Chargement…" else "Rechercher le DTC") }
            }
            item {
                OutlinedButton(
                    onClick = {
            ...
            guide?.let { d ->
                ...
                item {
                    Button(
                        onClick = ::runAi,
                        enabled = !aiLoading && !loading,
                        modifier = Modifier.fillMaxWidth()
                    ) { Text(if (aiLoading) "Analyse AI en cours…" else "Analyser avec CarDiag AI") }
                }
            }
...
@Composable
private fun AiResultCard(raw: String) {
    ...
    Card(Modifier.fillMaxWidth()) {
        Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(9.dp)) {
            ...
            Text(
                "L'AI assiste le diagnostic; les mesures OBD et les contrôles physiques restent nécessaires avant remplacement d'une pièce.",
                color = MaterialTheme.colorScheme.onSurfaceVariant
            )
        }
    }
