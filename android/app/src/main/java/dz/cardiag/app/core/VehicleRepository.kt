...
class VehicleRepository {
    ...
    suspend fun getVehicleProfile(modelId:String):List<VehicleYearProfile>{
        ...
        return years.sortedByDescending{it.modelYear}.map{year->
            ...
            val trimLinks=runCatching{supabase.from("vehicle_year_trim_links").select(Columns.list("model_year_id","trim_id")){filter{eq("model_year_id",year.id)}}.decodeList<YearTrimLinkRow>()}.getOrDefault(emptyList())
            ...
            val ecus=ecuLinks.mapNotNull{link->runCatching{supabase.from("vehicle_ecus").select(Columns.list("id","generation_id","engine_id","ecu_id","required","year_from","year_to","notes")){filter{eq("id",link.ecuId)}}.decodeSingle<VehicleEcuRow>()}.getOrNull()?.let{ecuLink->runCatching{supabase.from("ecu_modules").select(Columns.list("id","manufacturer","name","family","ecu_type","part_numbers","protocols")){filter{eq("id",ecuLink.ecuId)}}.decodeSingle<EcuModuleRow>().getOrNull()?.let{ecuLink to it}}}
