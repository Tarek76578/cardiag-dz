...
@Serializable private data class YearTrimLinkRow(@SerialName("model_year_id") val modelYearId:String,@SerialName("trim_id") val trimId:String)
...
class VehicleRepository {
    ...
    suspend fun getVehicleProfile(modelId:String):List<VehicleYearProfile>{
        ...
        return years.sortedByDescending{it.modelYear}.map{year->
            ...
            val trimLinks=supabase.from("vehicle_year_trim_links").select(Columns.list("model_year_id","trim_id")){filter{eq("model_year_id",year.id)}}.decodeList<YearTrimLinkRow>()
            val trims=trimLinks.mapNotNull{link->runCatching{supabase.from("vehicle_trims").select(Columns.list("id","generation_id","engine_id","name","code","drivetrain","transmission","doors","seats","year_from","year_to","market")){filter{eq("id",link.trimId)}}.decodeSingle<VehicleTrimRow>()}.getOrNull()}.filter{generationId==null || it.generationId==generationId}.filter{it.engineId==null||it.engineId in engineIds}.filter{containsYear(it.yearFrom,it.yearTo,year.modelYear)}
            val trimIds=trims.map{it.id}.toSet()
            val specLinks=supabase.from("vehicle_year_specification_links").select(Columns.list("model_year_id","specification_id")){filter{eq("model_year_id",year.id)}}.decodeList<YearSpecLinkRow>()
            ...
            val ecuLinks=supabase.from("vehicle_year_ecu_links").select(Columns.list("model_year_id","ecu_id")){filter{eq("model_year_id",year.id)}}.decodeList<YearEcuLinkRow>()
            val ecus=ecuLinks.mapNotNull{link->runCatching{supabase.from("vehicle_ecus").select(Columns.list("id","generation_id","engine_id","ecu_id","required","year_from","year_to","notes")){filter{eq("id",link.ecuId)}}.decodeSingle<VehicleEcuRow>()}.mapNotNull{ecuLink->runCatching{supabase.from("ecu_modules").select(Columns.list("id","manufacturer","name","family","ecu_type","part_numbers","protocols")){filter{eq("id",ecuLink.ecuId)}}.decodeSingle<EcuModuleRow>().getOrNull()?.let{ecuLink to it}}}
