...
object ObdParser {
    ...
    private fun response(bytes:List<Int>,mode:Int,pid:Int):Int { val i=bytes.windowed(2).indexOfFirst{it[0]==mode+0x40&&it[1]==pid}; return i }
    ...
    fun parseDtc(raw:String, mode:Int=0x43):List<String>{val b=bytes(raw);val i=b.indexOf(mode);if(i<0)return emptyList();return b.drop(i+1).chunked(2).takeWhile{it.size==2&&!(it[0]==0&&it[1]==0)}.map{p->val c=(p[0] shl 8)or p[1];val q=when((c shr 14)and 3){0->'P';1->'C';2->'B';else->'U'};"$q${(c shr 12)and 3}${c.toString(16).uppercase().padStart(4,'0').takeLast(3)}"}.distinct()}
    fun parseSupportedPids(raw:String,base:Int):Set<Int>{val b=bytes(raw);val i=response(b,1,0x00+base);if(i<0||i+4>=b.size)return emptySet();val mask=(b[i+2] shl 24)or(b[i+3] shl 16)or(b[i+4] shl 8)or(if(i+5<b.size)b[i+5] else 0);return (1..32).filter{mask and (1 shl (32-it))!=0}.map{base+it}.toSet()}
    ...
    fun parseReadiness(raw:String):ReadinessStatus{val b=bytes(raw);val i=response(b,1,0x01);if(i<0||i+6>=b.size)return ReadinessStatus(null,null,normalize(raw));val mil=(b[i+2] and 0x80)!=0;val monitors=(b[i+2] and 0x7F)==0;return ReadinessStatus(mil,monitors,normalize(raw))}
}
