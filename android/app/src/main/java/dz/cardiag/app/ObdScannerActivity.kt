 ...
 * The actual UX lives in [CarDiagUnifiedApp] under [CarDiagRoute.OBD].
 */
class ObdScannerActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        val route = runCatching { CarDiagRoute.valueOf(intent.getStringExtra(CarDiagActivityRoute.EXTRA_INITIAL_ROUTE) ?: CarDiagRoute.OBD.name) }.getOrDefault(CarDiagRoute.OBD)
        setContent {
            CarDiagUnifiedApp(
                initialRoute = route,
