...
private fun ObdScannerScreen(obd: ObdService, modelId: String?, modelName: String?, targetDtc: String?) {
    ...
    fun refresh() {
        if (ContextCompat.checkSelfPermission(context, Manifest.permission.BLUETOOTH_CONNECT) != PackageManager.PERMISSION_GRANTED) {
            (context as? ComponentActivity)?.requestPermissions(arrayOf(Manifest.permission.BLUETOOTH_SCAN, Manifest.permission.BLUETOOTH_CONNECT), 4101); return
        }
    ...
    fun clearDtcs() {
        scope.launch {
            ...
            runCatching { obd.clearTroubleCodes() }
                .onSuccess { dtcs = emptyList(); pending = emptyList(); status = "Effacement DTC envoyé — rescanner recommandé" }
                .onFailure { status = it.message ?: "Clear DTC impossible" }
            busy = false
        }
    }
    ...
    Scaffold(topBar = { TopAppBar(title = { Text("Scanner OBD-II Pro") }) }) { padding ->
        LazyColumn(Modifier.fillMaxSize().padding(padding), contentPadding = PaddingValues(18.dp), verticalArrangement = Arrangement.spacedBy(14.dp)) {
            ...
            if (!connected) {
                ...
                item { Button(onClick = { showClearDialog = true }, enabled = !busy, modifier = Modifier.fillMaxWidth(), colors = ButtonDefaults.buttonColors(containerColor = MaterialTheme.colorScheme.error)) { Text("EFFACER DTC") } }
    ...
    if (showClearDialog) {
        AlertDialog(
            ...
            title = { Text("Effacer les codes défaut ?") },
            text = { Text("Cette action modifie l'état du véhicule et peut effacer des informations utiles au diagnostic et remettre les moniteurs d'émissions à l'état non prêt. Ne l'utilisez qu'après avoir enregistré les codes et le freeze frame.") },
            confirmButton = { TextButton(onClick = { showClearDialog = false; clearDtcs() }, enabled = !busy) { Text("Effacer") } },
            dismissButton = { TextButton(onClick = { showClearDialog = false }, enabled = !busy) { Text("Annuler") } }
        )
    }
}
