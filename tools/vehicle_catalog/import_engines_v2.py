...
def call(endpoint, key, start):
    ...
    raise SystemExit(f"Canonical importer failed at source offset {start} after {MAX_RETRIES} attempts: {last}")
...
def main():
    ...
    for start in range(0, TOTAL_SOURCE_ROWS, BATCH_SIZE):
        result = call(endpoint, key, start)
    ...
    if total_failed:
        raise SystemExit(f"Canonical importer finished with {total_failed} failed row writes")
